...
export function Pagination({ allPokemons, currentPage }: AppProps) {
  ...
  const endPage = Math.min(
    ...
    filterPokemons(allPokemons, currentPage, storedValue, true).length
  );

  if (
    filterPokemons(allPokemons, currentPage, storedValue, true).length === 0 ||
  ...
  return (
    <section className="pagination-container">
      ...
      <div className="pagination">
        {filterPokemons(allPokemons, currentPage, storedValue, true)
      ...
      <button
        disabled={
          currentGroup === filteredPokemons.slice(startPage, endPage).length - 1
        }
